...
/************************************************************************************************/
	...
	if (!bInhibitUnits && plot()->getNumDefenders(getOwnerINLINE()) == 0) // XXX check for other team's units?
	{
		...
		if (AI_chooseUnit("defenseless city", UNITAI_CITY_DEFENSE, -1, -1, CITY_BUILD_PRIORITY_CEILING, &criteria))
		{
			return;
		}
/********************************************************************************/
...
/********************************************************************************/
		...
		if (AI_chooseUnit("defenseless city counter units", UNITAI_CITY_COUNTER, -1, -1, CITY_BUILD_PRIORITY_CEILING, &criteria))
		{
			return;
		}

		if (AI_chooseUnit("defenseless city special units", UNITAI_CITY_SPECIAL, -1, -1, CITY_BUILD_PRIORITY_CEILING, &criteria))
		{
			return;
		}
...
/* 	City Defenders						24.07.2010				Fuyu			*/
/********************************************************************************/
		if (AI_chooseUnit("defenseless city reserve units", UNITAI_RESERVE, -1, -1, CITY_BUILD_PRIORITY_CEILING, &criteria))
		{
			return;
		}
...
/********************************************************************************/
		...
		if (AI_chooseUnit("defenseless city (accept attack units)", UNITAI_ATTACK, -1, -1, CITY_BUILD_PRIORITY_CEILING, &criteria))
		{
			return;
		}
	}
...
/************************************************************************************************/
	if (!bInhibitUnits && (GC.getGameINLINE().isOption(GAMEOPTION_RUTHLESS_AI) || kPlayer.isEnabledMAD() || (getCitySorenRandNum(10, "AI consider Nuke") == 0)))
	{
		if(!bFinancialTrouble)
		{
...
/************************************************************************************************/
	...
	if (!bInhibitUnits && (kPlayer.AI_isDoStrategy(AI_STRATEGY_OWABWNW) || kPlayer.isEnabledMAD() || (getCitySorenRandNum(chance, "AI consider Nuke") == 0)))
	{
		if( !bFinancialTrouble )
		{
			int iTotalNukes = kPlayer.AI_totalUnitAIs(UNITAI_ICBM);
...
BuildingTypes CvCityAI::AI_bestBuildingThreshold(int iFocusFlags, int iMaxTurns, int iMinThreshold, bool bAsync, AdvisorTypes eIgnoreAdvisor, bool bMaximizeFlaggedValue, PropertyTypes eProperty)
{
	...
	for (iI = 0; iI < GC.getNumBuildingClassInfos(); iI++)
	{
		if (!(GET_PLAYER(getOwnerINLINE()).isBuildingClassMaxedOut(((BuildingClassTypes)iI), GC.getBuildingClassInfo((BuildingClassTypes)iI).getExtraPlayerInstances())))
		{
			...
			if ((eLoopBuilding != NO_BUILDING) && (getNumBuilding(eLoopBuilding, true) < GC.getCITY_MAX_NUM_BUILDINGS())
					...
					if ((eIgnoreAdvisor == NO_ADVISOR) || (GC.getBuildingInfo(eLoopBuilding).getAdvisorType() != eIgnoreAdvisor))
					{
						if (canConstruct(eLoopBuilding))
						{
							...
							else
							{
								...
								bBuildingInvalid = false;

								if (iFocusFlags & BUILDINGFOCUS_PROPERTY && eProperty != NO_PROPERTY)
								{
									CvPropertyManipulators* pMani = GC.getBuildingInfo(eLoopBuilding).getPropertyManipulators();
									...
									for (int i=0; i<iNum; i++)
									{
										...
										else if ( pSource->getType() == PROPERTYSOURCE_CONSTANT )
										{
											//	For now we're only interested in constant sources
											//	TODO - expand this as buildings add other types
											int iPropValue = GC.getPropertyInfo(eProperty).getAIWeight()*((CvPropertySourceConstant*)pSource)->getAmountPerTurn(getGameObjectConst());
											if (iPropValue>0)
											{
								...
								if (!bBuildingInvalid)
								{
									...
									if ( gCityLogLevel > 3 )
									{
										logBBAI("City %S base value for %S (flags %08lx)=%d",
												 getName().GetCString(),
											...
											if (eJLoopBuilding != NO_BUILDING)
											{
												if (GC.getBuildingInfo(eJLoopBuilding).isReplaceBuildingClass(iI))
												{
													if (getNumBuilding(eJLoopBuilding) > 0)
													{
														...
														if ( gCityLogLevel > 3 )
														{
															logBBAI("    replaces %S - reduce value to %d",
									...
									for (iJ = 0; iJ < GC.getNumBuildingInfos(); iJ++)
									{
											...
											GC.getBuildingInfo((BuildingTypes)iJ).isPrereqOrBuildingClass(iI))
										{
												 ...
												 canConstructInternal((BuildingTypes)iJ, false, false, false, true, (BuildingClassTypes)iI) )
											{
												...
												if ( gCityLogLevel > 3 )
												{
													logBBAI("    enables %S - increase value to %d",
...
/************************************************************************************************/
								...
								if ( bUnitIsEnabler )
								{
									...
									switch (eUnitAI)
									{
										...
										case UNITAI_EXPLORE_SEA:
											...
											break;
										case UNITAI_ICBM:
											iTempValue += (kUnit.getNukeRange() != -1 ? kUnit.getNukeRange() * 50: 0);
											break;
									}
									if (bWarPlan)
										iTempValue *= 2;
	...
	*/
	...
	/************************************************************************************************/
					...
					for (iJ = 0; iJ < kBuilding.getNumUnitCombatProdModifiers(); iJ++)
					{
	...
	/************************************************************************************************/
				...
				else
				{
					...
					if (iFocusFlags & BUILDINGFOCUS_PRODUCTION)
					{
						...
						iTempValue = AI_buildingYieldValue(YIELD_PRODUCTION, eBuilding, kBuilding, bForeignTrade, iFoodDifference, aiFreeSpecialistYield[YIELD_PRODUCTION]);
						...
						if ( iTempValue > getBaseYieldRate(YIELD_PRODUCTION)/3 )
						{
							...
							int iBoost = (iTempValue*12 - getBaseYieldRate(YIELD_PRODUCTION)*4);

							//	Examples -
							//	1) City production is 1, building adds 1 (iTempValue 8), so iBoost is 92 - value almost trebbled
	...
	/************************************************************************************************/	
				...
				if (iPass > 0)
				{
					...
					for (iI = 0; iI < NUM_COMMERCE_TYPES; iI++)
					{
						...
						for (iJ = 0; iJ < GC.getNumSpecialistInfos(); iJ++)
						{
							iTempValue += ((kBuilding.getSpecialistCommerceChange(iJ, iI) * kOwner.getTotalPopulation()) / 6);
						}
						if (iTempValue != 0)
						{
...
/************************************************************************************************/
	...
	for (iI = 0; iI < NUM_COMMERCE_TYPES; iI++)
	{
		iTempValue = GC.getProcessInfo(eProcess).getProductionToCommerceModifier((CommerceTypes)iI);
	...
	for (int iI = 0; iI < GC.getNumProcessInfos(); iI++)
	{
		if (canMaintain((ProcessTypes)iI))
		{
			...
			for (int iJ = 0; iJ < NUM_COMMERCE_TYPES; iJ++)
			{
				...
				iTemp = GC.getProcessInfo(eProcess).getProductionToCommerceModifier(eCommerce);
...
bool CvCityAI::AI_chooseBuilding(int iFocusFlags, int iMaxTurns, int iMinThreshold, int iOdds, bool bMaximizeFlaggedValue, PropertyTypes eProperty)
{
	...
	eBestBuilding = AI_bestBuildingThreshold(iFocusFlags, iMaxTurns, iMinThreshold, false, NO_ADVISOR, bMaximizeFlaggedValue, eProperty);
...
bool CvCityAI::buildingMayHaveAnyValue(BuildingTypes eBuilding, int iFocusFlags)
{
	...
	if ( (iFocusFlags & BUILDINGFOCUS_HAPPY) != 0 )
	{
		...
		if ((kBuilding.getNumTechHappinessTypes() > 0) ||
			(kBuilding.getNumTechSpecialistHappinesses() > 0))
		{
			for (int iI = 0; iI < GC.getNumTechInfos(); iI++)
			{
				if (GET_TEAM(getTeam()).isHasTech((TechTypes)iI))
				{
					...
					if (kBuilding.getTechSpecialistHappiness(iI) > 0)
					{
						bTechHapp = true;
					}
				}
		...
		if ( kBuilding.getHappiness() > 0 ||
			 ...
			 bTechHapp ||
			 kBuilding.isAnyTechSpecialistHappinessType())
		{
			return true;
		}
	}
	if ( (iFocusFlags & BUILDINGFOCUS_HEALTHY) != 0 )
	{
		...
		if ((kBuilding.getNumTechHealthTypes() > 0) ||
			(kBuilding.getNumTechSpecialistHealths() > 0))
		{
			for (int iI = 0; iI < GC.getNumTechInfos(); iI++)
			{
				if (GET_TEAM(getTeam()).isHasTech((TechTypes)iI))
				{
					...
					if (kBuilding.getTechSpecialistHealth(iI) > 0)
					{
						bTechHealth = true;
					}
				}
		...
		if ( kBuilding.getHealth() > 0 ||
			 ...
			 bTechHealth ||
			 kBuilding.isAnyTechSpecialistHealthType()
			 )
		{
			return true;
		}
...
void CvCityAI::CalculateAllBuildingValues(int iFocusFlags)
{
	...
	
	{
		...
		for (iBuilding = 0; iBuilding < numNumBuildingClassInfos; iBuilding++)
		{
			...
			if (NO_BUILDING != eBuilding)
			{
				...
				for (iJ = 0; iJ < numNumBuildingClassInfos; iJ++)
				{
					BuildingTypes eJLoopBuilding = ((BuildingTypes)(GC.getBuildingClassInfo((BuildingClassTypes)iJ).getDefaultBuildingIndex()));
...
bool CvCityAI::AI_choosePropertyControlUnit(int iTriggerPercentOfPropertyOpRange, PropertyTypes pProperty)
{
	...
	for( int iI= 0; iI < GC.getNumPropertyInfos(); iI++)
	{
		...
		if (pProperty == NO_PROPERTY || pProperty == eProperty)
		{
			...
			{
				iCurrentValue = getPropertiesConst()->getValueByProperty(eProperty);
				...
				if (iEval > iCheck && iCurrentPercent < iLimit )
				{
					...
					if (AI_isNavalMilitaryProductionCity())
					{
						...
						{
							iResponders = GET_PLAYER(getOwnerINLINE()).getContractBroker().numRequestsOutstanding(UNITAI_PROPERTY_CONTROL_SEA, false, pPlot);
							if (iResponders < 1)
							{	
					...
					if (!bSuccessful)
					{
						...
						if ( m_iUnitSummonedLastEstablishedTurn != iGameTurn )
						{
							iResponders = GET_PLAYER(getOwnerINLINE()).getContractBroker().numRequestsOutstanding(UNITAI_PROPERTY_CONTROL, false, pPlot);
							...
							else
							{
								...
								eBestUnit = AI_bestUnitAI(UNITAI_PROPERTY_CONTROL, iUnitValue, false, false, &criteria);	
								if (eBestUnit != NO_UNIT)
								{
...
bool CvCityAI::AI_establishSeeInvisibleCoverage()
{
	...
	if (pPlot != NULL)
	{
		for (int iI = 0; iI < GC.getNumInvisibleInfos(); iI++)
		{
			...
			//if (eBestUnit != NO_UNIT)
			//{
			iResponders = GET_PLAYER(getOwnerINLINE()).getContractBroker().numRequestsOutstanding(UNITAI_SEE_INVISIBLE, false, pPlot);//This will tend to space out the AI's sense of immediate need to prepare for each type.
			iResponders += pPlot->countSeeInvisibleActive(getOwner(), eVisible);
			if (iResponders < 1)
			{			
				//end result
