...
void CvMessageControl::sendBuildListPushOrder(int iID, OrderTypes eOrder, int iData, bool bAlt, bool bShift, bool bCtrl)
{
	if (NO_PLAYER != GC.getGameINLINE().getActivePlayer())
	{
		gDLL->sendMessageData(new CvNetBuildListPushOrder(GC.getGameINLINE().getActivePlayer(), iID, eOrder, iData, bAlt, bShift, bCtrl));
	}
}
...
void CvMessageControl::sendBuildListPopOrder(int iID, int iNum)
{
	if (NO_PLAYER != GC.getGameINLINE().getActivePlayer())
	{
		gDLL->sendMessageData(new CvNetBuildListPopOrder(GC.getGameINLINE().getActivePlayer(), iID, iNum));
	}
}
...
void CvMessageControl::sendBuildListEdit(int iID, CvString szName, bool bDelete)
{
	if (NO_PLAYER != GC.getGameINLINE().getActivePlayer())
	{
		gDLL->sendMessageData(new CvNetBuildListEdit(GC.getGameINLINE().getActivePlayer(), iID, szName, bDelete));
	}
}
...
void CvMessageControl::sendChooseBuildUp(PromotionLineTypes ePromotionLine, int iID)
{
	if (NO_PLAYER != GC.getGameINLINE().getActivePlayer())
	{
		gDLL->sendMessageData(new CvNetChooseBuildUp(GC.getGameINLINE().getActivePlayer(), ePromotionLine, iID));
	}
}
...
void CvMessageControl::sendChooseTrait(TraitTypes eTrait, bool bNewValue)
{
	if (NO_PLAYER != GC.getGameINLINE().getActivePlayer())
	{
		gDLL->sendMessageData(new CvNetChooseTrait(GC.getGameINLINE().getActivePlayer(), eTrait, bNewValue));
	}
}
...
void CvMessageControl::sendMergeUnit(int iUnitID)
{
	if (NO_PLAYER != GC.getGameINLINE().getActivePlayer())
	{
		gDLL->sendMessageData(new CvNetChooseMergeUnit(GC.getGameINLINE().getActivePlayer(), iUnitID));
	}
}
...
void CvMessageControl::sendSplitUnit(bool bConfirm)
{
	if (NO_PLAYER != GC.getGameINLINE().getActivePlayer())
	{
		gDLL->sendMessageData(new CvNetConfirmSplitUnit(GC.getGameINLINE().getActivePlayer(), bConfirm));
	}
}
...
void CvMessageControl::sendImprovementUpgrade(ImprovementTypes eImprovement, int iX, int iY, bool bConfirm)
{
	if (NO_PLAYER != GC.getGameINLINE().getActivePlayer())
	{
		gDLL->sendMessageData(new CvNetImprovementUpgrade(GC.getGameINLINE().getActivePlayer(), eImprovement, iX, iY, bConfirm));
	}
}
...
void CvMessageControl::sendArrestUnit(int iUnitID, PlayerTypes ePlayerT)
{
	if (NO_PLAYER != GC.getGameINLINE().getActivePlayer())
	{
		gDLL->sendMessageData(new CvNetChooseArrestUnit(GC.getGameINLINE().getActivePlayer(), ePlayerT, iUnitID));
	}
}
...
void CvMessageControl::sendAmbushConfirmation(bool bConfirm)
{
	if (NO_PLAYER != GC.getGameINLINE().getActivePlayer())
	{
		gDLL->sendMessageData(new CvNetConfirmAmbush(GC.getGameINLINE().getActivePlayer(), bConfirm));
	}
}