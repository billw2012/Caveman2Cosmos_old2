...
{
	InitializeCriticalSectionAndSpinCount(&m_cFreeTradeRegionBuildingsSection, 4000);
...
void CvPlotGroup::changeNumFreeTradeRegionBuildings(BuildingTypes eBuilding, int iChange)
{
	...
	if (iChange != 0)
	{
		EnterCriticalSection(&m_cFreeTradeRegionBuildingsSection);
		...
		LeaveCriticalSection(&m_cFreeTradeRegionBuildingsSection);
