...
CvArea::~CvArea()
{
	...
	DeleteCriticalSection(&m_cPlotTypeCacheSection);
}
...
void CvArea::changePower(PlayerTypes eIndex, int iChange)
{
	...
	} while( InterlockedCompareExchange((volatile LONG*)&m_aiPower[eIndex], iOldValue + iChange, iOldValue) != iOldValue );
...
int CvArea::getNumRevealedFeatureTiles(TeamTypes eTeam, FeatureTypes eFeature) const
{
	...
	bool bHaveCachedResult = (itr == m_plotFeatureCountCache.end());
...
void CvArea::changeNumTrainAIUnits(PlayerTypes eIndex1, UnitAITypes eIndex2, int iChange)
{
	...
	do
	{
		iOldValue = m_aaiNumTrainAIUnits[eIndex1][eIndex2];
	} while( InterlockedCompareExchange((volatile LONG*)&m_aaiNumTrainAIUnits[eIndex1][eIndex2], iOldValue + iChange, iOldValue) != iOldValue );
