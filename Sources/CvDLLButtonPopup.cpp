	...
	case BUTTONPOPUP_CHOOSE_ARREST_UNIT:
		if (pPopupReturn->getButtonClicked() != 0)
		{
			...
			while (pUnitNode != NULL)
			{
				...
				if (pLoopUnit->getID() == pPopupReturn->getButtonClicked())
				{
					ePlayerT = pLoopUnit->getOwner();
					break;
				}
			}
			if (ePlayerT != NO_PLAYER)
			{
				CvMessageControl::getInstance().sendArrestUnit(pPopupReturn->getButtonClicked(), ePlayerT);
			}
		}
		else
		{
		...
		if (eLoopUnit != NO_UNIT)
		{
			if (eLoopUnit != eProductionUnit)
			{
				if (pCity->canTrain(eLoopUnit))
				{
					...
					{
						//TB Debug Note/Question:
						//I'm not sure where the formatted display is being delivered but it's clear that this is where the
						//SubCombats were causing additional units to go into the list.  I attempted to setup another sz definition that
						//could be appended to from the original formatting here but I could not get .append to work in this file at all
						//additionally, it appears that the szBuffer is completely redefined before the delivery of the formated text anyhow
						//and this becomes merely a way to define the unit as trainable.  Some clarification on how and if we should attempt
						//to work in the additional sub combat values somehow would be beneficial at some point
						//But at least this commented out reference to the subcombat type fixes the bug.
						UnitCombatTypes eUnitCombat = ((UnitCombatTypes)iJ);
						if ((UnitCombatTypes)GC.getUnitInfo(eLoopUnit).getUnitCombatType() == eUnitCombat/* ||
							GC.getUnitInfo(eLoopUnit).isSubCombatType(eUnitCombat)*/)
						{
							int iTurns = pCity->getProductionTurnsLeft(eLoopUnit, 0);
							szBuffer.Format(L"%d %s (%d) - [%d]", iJ, GC.getUnitInfo(eLoopUnit).getDescription(), iTurns, iI);
