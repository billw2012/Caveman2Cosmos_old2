...
//	the pipeline.  Approriate subclasses should be defined for concrete items, sufficient
//	to record their state chnages as they flow through the pipeline stages
...
//	asynchronous.  A synchronous stage will not begin executign until all workflow items
...
//	A stage may have mutliple threads allocated to its processing if desired
...
protected:
	void	NoteProcessed(CvPipelineWorkItem* item);
	void	Stall(void);
...
//	A workflow pipeline the processes work items through multiple pipeline processing stages
class CvPipeline
{
friend CvPipelineWorkItem;
...
public:
	...
	//	Note - a stage is assumed to be owned by the pieline once added, and will be deleted
	//	by it when it is finished with it.  FUTURE - encapsulate better withn a stage factory
...
//	A no-op synchronous stage is often usefulto provide a synchronization point to ensure
//	all previous pipeline stages are compleet fro earlier stages before the next (async) stage begins
//	this class serves that prupose
