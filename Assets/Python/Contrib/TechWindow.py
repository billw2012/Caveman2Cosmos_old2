...
#		self.Y_QUOTE = self.Y_UPPER_PANEL + self.iMarginSpace + 36
		...
		self.W_QUOTE = 455                                                                                               #  Spocko was 400
...
#		screen.setHelpTextArea(self.W_HELP_AREA, FontTypes.SMALL_FONT, self.X_UNIT_PANE, self.Y_UNIT_PANE, self.Z_HELP_AREA, 1, CyArtFileMgr().getInterfaceArtInfo("POPUPS_BACKGROUND_TRANSPARENT").getPath(), True, True, CvUtil.FONT_LEFT_JUSTIFY, 0 )
		...
		screen.addPanel( szSpecialPanel, "", "", True, True,
				self.X_SPECIAL_PANEL+self.iMarginSpace, self.Y_SPECIAL_PANEL, self.W_SPECIAL_PANEL-(self.iMarginSpace * 2), self.H_SPECIAL_PANEL + 5, PanelStyles.PANEL_STYLE_IN )
...
#---Eingefügt START - kann komplett gelöscht werden-----------------
		...
		# Allows Panel4
		panelName4 = "SIR4"
		screen.addPanel( panelName4, "", "", False, True,
                                 self.X_ALLOWS_PANEL+self.iMarginSpace, self.Y_ALLOWS_PANEL4, self.W_ALLOWS_PANEL-(self.iMarginSpace * 2), self.H_ALLOWS_PANEL, PanelStyles.PANEL_STYLE_IN )
...
#---Eingefügt ENDE -------------------------------------------------
		...
		screen.addMultilineText(listName, szSpecialText, self.X_SPECIAL_PANEL+20, self.Y_SPECIAL_PANEL+20, self.W_SPECIAL_PANEL-50, self.H_SPECIAL_PANEL-30, WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
...
#---Eingefügt START - kann komplett gelöscht werden --------------
		...
		screen.setText(szAllowsTitleWidgetSIR, "", szAllowsTitleDescSIR, CvUtil.FONT_LEFT_JUSTIFY,
			       self.X_ALLOWS_PANEL + self.iMarginSpace, self.Y_ALLOWS_PANEL4 - 20, 0, FontTypes.TITLE_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)
		...
		for j in range(CyGlobalContext().getNumTechInfos()):
			for k in range(CyGlobalContext().getDefineINT("NUM_OR_TECH_PREREQS")):
				...
				if (iPrereq == self.iTech):
        				screen.attachImageButton( panelName4, "", CyGlobalContext().getTechInfo(j).getButton(), GenericButtonSizes.BUTTON_SIZE_CUSTOM, WidgetTypes.WIDGET_PEDIA_JUMP_TO_DERIVED_TECH, j, self.iTech, False )
