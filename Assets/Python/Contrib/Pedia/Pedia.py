...
#			"RANDOM"		: RANDOM
		...
		iWidMain	= WidgetTypes.WIDGET_PEDIA_MAIN
		...
		screen.addDDSGFC("PediaMainBackground", CyArtFileMgr().getInterfaceArtInfo("SCREEN_BG_OPAQUE").getPath(), 0, 0, xRes, yRes, iWidGen, -1, -1)
		...
		screen.setText("PediaMain|Header", "", HEAD_TEXT, 1<<0, 16, 0, 0, iFontTitle, iWidGen, 1, 1)
		screen.setText("PediaMain|Contents", "", TOC_TEXT, 1<<0, 16, 0, 0, iFontTitle, iWidGen, -1, -1)
		screen.setText("PediaMain|Index", "", INDEX_TEXT, 1<<0, 16, Y_BOT_TEXT, 0, iFontTitle, iWidGen, -1, -1)
		...
		for i, aList in enumerate(categoryList):
			...
			screen.appendListBoxStringNoUpdate(CAT_LIST_ID, graphic + aList[1], iWidMain, i, 0, 1<<0)
	...
	def pediaJump(self, iCategory, szSubCat, iObjectType, bRemoveFwdList = True):
		...
		if bConcept:
			...
			self.mapScreenFunctions.get(iGroupCategory).interfaceScreen(iObjectType)
		elif bFuncByCat:
			self.mapScreenFunctions.get(iCategory).interfaceScreen(iObjectType)
		elif bFuncBySubCatSimple:
			self.mapScreenFunctions.get(szSubCatSimple).interfaceScreen(iObjectType)
		else:
			self.mapScreenFunctions.get(szSubCat).interfaceScreen(iObjectType)
	...
	def showContents(self, screen, iCategory, aSubCatList, szSubCat):
		...
		for i in range(len(aSubCatList)):
			if bDrawSubCat:
				screen.appendListBoxStringNoUpdate(SUBCAT_LIST_ID, szfont3b + aSubCatList[i], iWidMain, i + 1000, 0, 1<<0)
	...
	def placeHints(self):
		...
		for hint in hintText:
			if len(hint) != 0:
				screen.appendListBoxStringNoUpdate(szHintBox, hint, iWidGen, -1, -1, 1<<0)
