...
#			'playerGoldTrade'			: self.onPlayerGoldTrade,
		...
		CvUtil.SilentEvents.extend([5000, 5006])
	###****************###
	### EVENT STARTERS ###
	...
	def onMouseEvent(self, argsList):
		...
		if eventType == self.EventLButtonDown:
			if px != -1 and py != -1:
				if self.bCtrl and self.bAllowCheats and not interfaceConsumed:
	...
	def onKbdEvent(self, argsList):
		...
		if eventType == 6: # Key down
			...
			if self.bAllowCheats:
				...
				elif key == InputTypes.KB_LBRACKET and bShift:
					unit = GC.getMap().plot(px, py).getUnit(0)
					if not unit.isNone():
						d = min(unit.maxHitPoints()-1, unit.getDamage() + 10)
						unit.setDamage(d, PlayerTypes.NO_PLAYER)
						return 1
				# Shift - [ (Debug - currently mouse-overd unit, health -= 10
				elif key == InputTypes.KB_RBRACKET and bShift:
					unit = GC.getMap().plot(px, py).getUnit(0)
					if not unit.isNone():
						d = max(0, unit.getDamage() - 10)
						unit.setDamage(d, PlayerTypes.NO_PLAYER)
						return 1
				elif key == InputTypes.KB_F1:
					if bShift:
					...
					if bShift:
						...
						return 1
				elif key == InputTypes.KB_F3:
					if bShift:
						CvScreensInterface.showDanQuayleScreen(())
						return 1
				elif key == InputTypes.KB_F4:
					if bShift:
		...
		if iPlayer in aWonderTuple[4]:
			for i, iPlayerX in enumerate(aWonderTuple[4]):
				...
				if KEY == "WORLD_BANK":
					...
					iWorldSize = GC.getMap().getWorldSize() + 1
					...
					for iPlayerX in xrange(self.MAX_PC_PLAYERS):
						...
						if not CyTeamX.isAlive() or CyTeamX.isVassal(iTeam):
							continue
						if TECH_SATELLITES > -1 and CyTeamX.isHasTech(TECH_SATELLITES):
							continue
						iChance = GAME.getSorenRandNum(5 * iWorldSize, "Zizkov")
						if not iChance:
							GC.getMap().setRevealedPlots(iTeamX, False, False)
							szText = TRNSLTR.getText("TXT_ZIZKOV1",(CyPlayerX.getCivilizationDescription(iPlayerX),))
							CyInterface().addMessage( iPlayer, True, 20, szText, '', 0, '', -1, -1, -1, False, False)
							szText = TRNSLTR.getText("TXT_ZIZKOV2",())
							CyInterface().addMessage(iPlayerX, True, 20, szText, '', 0, '', -1, -1, -1, False, False)
	...
	def onCombatResult(self, argsList):
		...
		if self.iWAR_PRIZES:
			...
			if iValid > -1 and CyUnitW.isHasUnitCombat(iValid) and CyUnitL.isHasUnitCombat(iValid):
				if GAME.getSorenRandNum(100, "WarPrize") <= 15:
					iUnitL = CyUnitL.getUnitType()
					X = CyUnitW.getX()
					Y = CyUnitW.getY()
					CyUnitWP = CyPlayerW.initUnit(iUnitL, X, Y, UnitAITypes.NO_UNITAI, DirectionTypes.NO_DIRECTION)
					...
					if bHumanW:
						colorGreen = ColorTypes(GC.getInfoTypeForString("COLOR_GREEN"))
						szText = TRNSLTR.getText("TXT_KEY_MISC_WARPRIZES_SUCCESS", (szWP,))
						artPath = CvUnitInfoL.getButton()
						CyInterface().addMessage(iPlayerW, False, 20, szText, '', 0, artPath, colorGreen, X, Y, True, True)
					x = CyUnitL.getX()
					y = CyUnitL.getY()
					if bHumanL:
						colorRed = ColorTypes(GC.getInfoTypeForString("COLOR_RED"))
						szText = TRNSLTR.getText("TXT_KEY_MISC_WARPRIZES_FAILURE", (szWP,))
						artPath = 'Art/Interface/Buttons/General/warning_popup.dds'
						CyInterface().addMessage(iPlayerL, False, 20, szText, '', 0, artPath, colorRed, x, y, True, True)
					...
					if iValid > -1 and CyUnitL.isHasUnitCombat(iValid):
						...
						if CyPlayerL.getGold() >= Loot:
							...
							if bHumanW:
								szText = TRNSLTR.getText("TXT_KEY_MISC_WARPRIZES_SUCCESS_GOLD_GAINED", (szWP,))
								artPath = 'Art/Interface/Buttons/process/processwealth.dds'
								CyInterface().addMessage(iPlayerW, False, 20, szText, '', 0, artPath, colorGreen, X, Y, True, True)
							if bHumanL:
								szText = TRNSLTR.getText("TXT_KEY_MISC_WARPRIZES_FAILURE_GOLD_LOST", (szWP,))
								artPath = 'Art/Interface/Buttons/General/warning_popup.dds'
								CyInterface().addMessage(iPlayerL, False, 20, szText, '', 0, artPath, colorRed, x, y, True, True)
		...
		if CyUnitL.isHasPromotion(iRespawn1) or bRespawn2:
			...
			if X == -1:
				X = CyUnitL.getX()
				Y = CyUnitL.getY()
			CyUnit = CyPlayerL.initUnit(iUnit, X, Y, UnitAITypes.NO_UNITAI, DirectionTypes.DIRECTION_NORTH)
			CyUnit.convert(CyUnitL)
			CyUnit.setDamage(GAME.getSorenRandNum(40, "XP") + 40, False)
			...
			if CyPlayerL.isHuman:
				artPath = 'Art/Interface/Buttons/Phoenix1.dds'
				...
				if bSneak:
					...
					if iStolen > 0:
						...
						if bHumanW:
							...
							CyInterface().addMessage(iPlayerW, False, 15, szText, '', 0, artPath, ColorTypes(44), X, Y, True, True)
						if bHumanL:
							szText = TRNSLTR.getText("TXT_KEY_SNEAK_FROM",(szLooser,iStolen))
							artPath = ',Art/Interface/Buttons/Process/ProcessCulture.dds,Art/Interface/Buttons/Beyond_the_Sword_Atlas.dds,8,16'
							CyInterface().addMessage(iPlayerL, False, 15, szText, '', 0, artPath, ColorTypes(44), X, Y, True, True)
				# Marauder promo
				if bMarauder:
	...
	def onBuildingBuilt(self, argsList):
		...
		if iBuilding in aWonderTuple[1]:
			...
			if KEY == "CRUSADE":
				...
				GC.getMap().setRevealedPlots(iTeam, True, False)
				...
				if bNetworkMP: # OOS guard
					from random import seed, shuffle
					seed(GAME.getMapRand().get(65535, "seed"))
				else:
				...
				for x in xrange(iX - 3, iX + 4):
					for y in xrange(iY - 3, iY + 4):
						CyPlot = GC.getMap().plot(x, y)
						if not CyPlot.isWater() or CyPlot.getBonusType(-1) != -1: continue
						aPlotList.append(CyPlot)
				shuffle(aPlotList)
				while aPlotList and aBonusList:
					BONUS = aBonusList.pop()
					if BONUS < 0: continue
					iRandom = GAME.getSorenRandNum(6, "TSUKIJI")
					if not iRandom:
						CyPlot = aPlotList.pop()
						CyPlot.setBonusType(BONUS)
						if BOAT > -1:
							CyPlot.setImprovementType(BOAT)
						szText = TRNSLTR.getText("TXT_TSUKIJI_BONUS",(GC.getBonusInfo(BONUS).getDescription(),))
						CyInterface().addMessage(iPlayer, True, 20, szText, '', 0, GC.getBonusInfo(BONUS).getButton(), ColorTypes(11), iX, iY, True, True)
		...
		# Djenne
			...
			for x in xrange(X - 2, X + 3, 1):
				for y in xrange(Y - 2, Y + 3, 1):
					CyPlot = MAP.plot(x, y)
		...
		# NANITE DEFUSER - destroyes all nukes from all players
			...
			for iPlayerX in xrange(self.MAX_PLAYERS):
				...
				while CyUnit:
					...
					CyUnit, i = CyPlayer.nextUnit(i, False)
				if CyPlayer.isHuman() and CyPlayer.isAlive():
					CyInterface().addMessage(iPlayerX, False, 15, szText, "", 0, 'art/interface/buttons/great_wonders/nanite_defuser.dds', ColorTypes(44), X, Y, True, True)
		# Machu Picchu
	...
	def onProjectBuilt(self, argsList):
		...
		if not self.bNetworkMP and CyCity.getOwner() == GAME.getActivePlayer():
			popupInfo = CyPopupInfo()
			popupInfo.setButtonPopupType(ButtonPopupTypes.BUTTONPOPUP_PYTHON_SCREEN)
			...
			popupInfo.setText('showWonderMovie')
			popupInfo.addPopup(CyCity.getOwner())
				...
				for y in xrange(iY - 50, iY + 50, 1):
					...
					if CyPlot.getTeam() == iTeam:
						...
						if iTerrain == GRASS:
							...
							CyPlot.setTerrainType(PLAINS, 1, 1)
			ID = CyPlayer.getID()
			CyInterface().addMessage(ID,False,15,TRNSLTR.getText("TXT_KEY_EDEN_PYTHON",()),'',0,'Art/Interface/Buttons/Buildings/Eden.dds',ColorTypes(44),iX,iY,True,True)
			...
			for i, iPlayerX in enumerate(aWonderTuple[4]):
				...
				if KEY == "ALAMO":
					...
					if CyUnit.isHasPromotion(GC.getInfoTypeForString("PROMOTION_LEADER")) or CyUnit.getUnitType() == GC.getInfoTypeForString("UNIT_GREAT_GENERAL"):
						...
						szText = TRNSLTR.getText("TXT_GG_REVIVE",(szName,))
						CyUnit = CyPlayerL.initUnit(GC.getInfoTypeForString("UNIT_GREAT_GENERAL"), x, y, UnitAITypes.NO_UNITAI, DirectionTypes.NO_DIRECTION)
						CyUnit.setName(szName)
						CyUnit.setImmobileTimer(8)
						CyInterface().addMessage(iPlayerL, True, 15, szText, '', 0, 'Art/Cyrus_Tomb/cyrustomb_button.dds', ColorTypes(11), x, y, True, True)
	...
	def onTechAcquired(self, argsList):
		...
		CyPlayer = GC.getPlayer(iPlayer)
		if CyPlayer.isNPC():
			print "NPC %s gained the technology %s" %(CyPlayer.getCivilizationDescription(0), GC.getTechInfo(iTech).getDescriptionForm(0))
			return
		...
		if self.GO_START_AS_MINORS:
			...
			if CyTeam.isMinorCiv() and GC.getTechInfo(iTech).isOpenBordersTrading():

				CyTeam.setIsMinorCiv(False, False)
				GC.getMap().verifyUnitValidPlot()

				for iPlayerX in range(self.MAX_PC_PLAYERS):
					CyPlayer = GC.getPlayer(iPlayerX)
					if CyPlayer.getTeam() == iTeam and CyPlayer.isAlive() and CyPlayer.isHuman():
						CyInterface().addMessage(
							iPlayerX, False, GC.getDefineINT("EVENT_MESSAGE_TIME"),
							TRNSLTR.getText("TXT_KEY_BARBCIV_DISCOVER_WRITING", ()),
							None, InterfaceMessageTypes.MESSAGE_TYPE_MAJOR_EVENT, None,
							GC.getInfoTypeForString("COLOR_HIGHLIGHT_TEXT"), -1, -1, False, False
						)
	...
	def onCityRazed(self, argsList):
		...
		if NumWonders:
			...
			if iPlayer == iActivePlayer:
				...
				iTeam = CyPlayer.getTeam()
				CyTeamActive = GC.getTeam(GC.getPlayer(iActivePlayer).getTeam())
				if CyTeamActive.isHasMet(iTeam):
					bDo = True
			if bDo:
				iColor = ColorTypes(GC.getInfoTypeForString("COLOR_RED"))
				szPlayerName = CyPlayer.getName()
				X = CyCity.getX()
				Y = CyCity.getY()
				for iBuilding in xrange(GC.getNumBuildingInfos()):
					if CyCity.getNumBuilding(iBuilding):
						...
						if GC.getBuildingClassInfo(CvBuildingInfo.getBuildingClassType()).getMaxGlobalInstances() == 1:
							WonderName = CvBuildingInfo.getDescription()
							if iPlayer == iActivePlayer:
								szText = TRNSLTR.getText("TXT_KEY_YOU_DESTROYED_WONDER", (0, WonderName))
							else:
								szText = TRNSLTR.getText("TXT_KEY_DESTROYED_WONDER", (szPlayerName, WonderName))
							CyInterface().addMessage(iActivePlayer, False, 15, szText, '', 0, 'Art/Interface/Buttons/General/warning_popup.dds', iColor, X, Y, True, True)
		# Partisans!
	...
	def onCityAcquiredAndKept(self, argsList):
		...
		if NumWonders:
			...
			if iPlayer == iActivePlayer:
				...
				iColor = ColorTypes(GC.getInfoTypeForString("COLOR_GREEN"))
			else:
				iTeam = CyPlayer.getTeam()
				CyTeamActive = GC.getTeam(GC.getPlayer(iActivePlayer).getTeam())
				if CyTeamActive.isHasMet(iTeam):
				...
				for iBuilding in xrange(GC.getNumBuildingInfos()):
					if CyCity.getNumBuilding(iBuilding):
						...
						if GC.getBuildingClassInfo(CvBuildingInfo.getBuildingClassType()).getMaxGlobalInstances() == 1:
							WonderName = CvBuildingInfo.getDescription ()
							if iPlayer == iActivePlayer:
								szText = TRNSLTR.getText("TXT_KEY_YOU_CAPTURED_WONDER", (0, WonderName))
							else:
								szText = TRNSLTR.getText("TXT_KEY_CAPTURED_WONDER", (szPlayerName, WonderName))
							CyInterface().addMessage(iActivePlayer, False, 15, szText, '', 0, artPath, iColor, X, Y, True, True)
		...
		if iCityID in aWonderTuple[3] and iPlayer in aWonderTuple[4]:
			for i, ID in enumerate(aWonderTuple[3]):
				...
				if KEY == "CRUSADE":
					...
					CyCity.addProductionExperience(CyUnit, False)
					if CyPlayer.isHuman():
						szText = TRNSLTR.getText("TXT_NEW_HORSE",(CyUnit.getName(),))
						CyInterface().addMessage(iPlayer, True, 20, szText, '', 0, CyUnit.getButton(), ColorTypes(11), X, Y, True, True)
					...
					CyCity.addProductionExperience(CyUnit, False)
					if CyPlayer.isHuman():
						szText = TRNSLTR.getText("TXT_NEW_ANIMAL",(CyUnit.getName(),))
						CyInterface().addMessage(iPlayer, True, 20, szText, '', 0, CyUnit.getButton(), ColorTypes(11), X, Y, True, True)
