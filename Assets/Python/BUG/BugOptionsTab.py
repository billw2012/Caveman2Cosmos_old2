...
class BugOptionsTab:
	...
	def __init__(self, name, title):
		...
		self.callbackIFace = "BUG_OptionsCB_IF"
	...
	def addLabel(self, screen, panel, name, title=None, tooltip=None, spacer=False, bCustom=False):
		...
		if spacer:
			...
			self.addSpacer(screen, hbox, hbox, 2)
			panel = hbox
		if title:
			label = name + "_Label"
	...
	def addTextEdit(self, screen, labelPanel, controlPanel, name, spacer=False):
		option = self.getOption(name)
		if option is not None:
			# create label
			if labelPanel == controlPanel:
				...
				if spacer:
					self.addSpacer(screen, box, box, 2)
				labelPanel = box
				controlPanel = box
			...
			screen.setToolTip(control, option.getTooltip())
			screen.setLayoutFlag(control, "LAYOUT_SIZE_HMIN")
			if not option.isEnabled():
				screen.setEnabled(control, False)
			return control
	...
	def addCheckboxDropdown(self, screen, checkPanel, dropPanel, checkName, dropName, layout, elements, index, callback, spacer=False):
		...
		if None not in (checkOption, dropOption):
			# create checkbox
